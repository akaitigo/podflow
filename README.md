...
> **Note**: 本番環境で公開する前に、以下の対応を確認してください:
...
- **フロントエンド**: Vitest で 47 テスト（コンポーネント・API・ビジネスロジック・認証）
- **バックエンド**: JUnit で 103 テスト（gRPC サービス・モデル・マッパー・ヘルスチェック・認証）
...
- **ADR（アーキテクチャ判断記録）**:
  ...
  - [ADR-005: JWT認証](docs/adr/005-jwt-authentication.md) — Quarkus SmallRye JWT による認証実装
