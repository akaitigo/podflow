...
6カラム（Planning → Guest Coordination → Recording → Editing → Review → Published）でエピソードの制作進捗を一覧管理。ドラッグ&ドロップでステータス変更。

### エピソード作成

![Create Episode](docs/images/create-modal.png)

タイトルと概要を入力して新規エピソードを作成。自動的に「Planning」ステータスで追加される。
